...
def setup_logger(level: int = logging.INFO) -> logging.Logger:
    ...
    return logging.getLogger("radiomics_feature_selection")
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if not (0.5 <= float(args.corr_thresh) <= 1.0):
        raise ValueError(f"corr_thresh must be in [0.5, 1.0], got {args.corr_thresh}")
    if args.mode == "fold":
        if int(args.n_splits) < 2:
            raise ValueError(f"n_splits must be >= 2 in fold mode, got {args.n_splits}")
        if not (0 <= int(args.fold_idx) < int(args.n_splits)):
            raise ValueError(
                f"fold_idx must be in [0, {int(args.n_splits) - 1}] for n_splits={args.n_splits}, "
                f"got {args.fold_idx}"
            )
