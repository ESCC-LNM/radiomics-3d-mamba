...
def setup_logger(level: int = logging.INFO) -> logging.Logger:
    ...
    return logging.getLogger("fusion_data_pipeline")
...
def get_args() -> argparse.Namespace:
    ...
    g_path.add_argument("--base_dir", type=Path, default=base_dir)
...
def get_fusion_dataloaders(args: argparse.Namespace) -> Dict[str, DataLoader]:
    ...
    if len(trainval_files) < int(args.n_splits):
        raise ValueError(
            f"Train/val samples ({len(trainval_files)}) must be >= n_splits ({args.n_splits})."
        )
    if len(np.unique(labels)) < 2:
        raise ValueError("Binary labels are required in the train/val pool, but only one class was found.")

    class_counts = np.bincount(labels)
    nonzero_counts = class_counts[class_counts > 0]
    if nonzero_counts.size > 0 and int(nonzero_counts.min()) < int(args.n_splits):
        raise ValueError(
            "At least n_splits samples are required in each class for stratified CV. "
