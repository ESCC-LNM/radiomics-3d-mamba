...
def setup_logger(output_dir: Path, level: int = logging.INFO) -> Path:
    """
    Configure root logging once the concrete output directory is known.

    This avoids import-time side effects and guarantees that file logging is not
    silently disabled by earlier library imports.
    """
    log_dir = output_dir.parent / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    log_file = log_dir / "train_fusion_mamba.log"
...
def save_run_metadata(args: argparse.Namespace, output_dir: Path, device: torch.device) -> Path:
    metadata = {
        "command": " ".join(sys.argv),
        "device": str(device),
        "effective_args": _json_ready(vars(args)),
        "environment": {
...
def validate_args(args: argparse.Namespace) -> None:
    if int(args.n_splits) < 2:
        raise ValueError(f"n_splits must be >= 2, got {args.n_splits}")
    if int(args.batch_size) < 1:
        raise ValueError(f"batch_size must be >= 1, got {args.batch_size}")
    if int(args.epochs) < 1:
        raise ValueError(f"epochs must be >= 1, got {args.epochs}")
    if int(args.rad_input_features) < 1:
        raise ValueError(f"rad_input_features must be >= 1, got {args.rad_input_features}")
    if getattr(args, "run_fold", None) is not None:
        run_fold = int(args.run_fold)
        if not (1 <= run_fold <= int(args.n_splits)):
            raise ValueError(f"run_fold must be in [1, {args.n_splits}], got {run_fold}")

    _require_exists(Path(args.internal_csv), "Internal metadata CSV")
    _require_exists(Path(args.external_csv), "External metadata CSV")
    _require_exists(Path(args.trainval_img_dir), "Train/val image directory")
    _require_exists(Path(args.internal_test_img_dir), "Internal test image directory")
    _require_exists(Path(args.external_test_img_dir), "External test image directory")

    if getattr(args, "rad_root_dir", None) is not None:
        _require_exists(Path(args.rad_root_dir), "Fold radiomics root directory")
    else:
        _require_exists(Path(args.rad_trainval_csv), "Selected radiomics train/val CSV")
        _require_exists(Path(args.rad_internal_test_csv), "Selected radiomics internal-test CSV")
        _require_exists(Path(args.rad_external_test_csv), "Selected radiomics external-test CSV")
...
def main(args: argparse.Namespace) -> None:
    ...
    log_file = setup_logger(Path(args.output_dir))
    validate_args(args)
    set_seed(int(getattr(args, "seed", 42)))
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    args.output_dir.mkdir(parents=True, exist_ok=True)
    metadata_path = save_run_metadata(args, Path(args.output_dir), device)

    if device.type != "cuda":
        log.warning("CUDA is not available; training will run on CPU and be very slow.")
    ...
    log.info(f"Run metadata        : {metadata_path}")
